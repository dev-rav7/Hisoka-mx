...
// Fonction pour demander le numéro de téléphone via l'interface console
const askForPhoneNumber = () => {
  return new Promise((resolve) => {
    ...
    rl.question('[ 📱 ] Entrez votre numéro WhatsApp (format: +33123456789): ', (phoneNumber) => {
      rl.close()
      ...
      if (cleanNumber.startsWith('+') && cleanNumber.length > 10) {
        ...
        console.log('[ ❌ ] Numéro invalide. Format attendu: +33123456789')
        resolve(null)
      }
    })
  })
...
async function connectToWA() {
  ...
  try {
    ...
    const sessionExists = await initializeSession()
    ...
    // Gestion du pairing code si pas de session
    if (!sessionExists && !conn.authState.creds.registered) {
      setTimeout(async () => {
        const phoneNumber = config.PHONE_NUMBER || await askForPhoneNumber()
        if (phoneNumber) {
          const code = await conn.requestPairingCode(phoneNumber)
          console.log(`[ 📱 ] Votre code de jumelage: ${code}`)
          console.log('[ 📋 ] Entrez ce code dans WhatsApp: Paramètres > Appareils liés > Lier un appareil')
        }
    ...
    conn.ev.on('connection.update', async (update) => {
      ...
      if (isNewLogin) {
        console.log('[ ✅ ] Nouvelle connexion établie via pairing code!')
      }
...
> © Powered by hhhisoka `
  ...
  if (conn) {
    addUtilityMethods(conn)
    setupEventHandlers()
    
    // Les credentials sont déjà sauvegardés automatiquement par saveCreds
  }
}
...
async function processMessage(mek, m) {
